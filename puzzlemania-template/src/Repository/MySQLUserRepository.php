    ...
    public function getUserById(int $id)
    {
        ...
        if ($count > 0) {
            ...
            $user = User::create()
                ->setId($row['id'])
                ->setEmail($row['email'])
                //->setPassword($row[0]['password']) - nope
                ->setCreatedAt(date_create_from_format('Y-m-d H:i:s', $row['createdAt']))
                ->setUpdatedAt(date_create_from_format('Y-m-d H:i:s', $row['updatedAt']));
            return $user;
        }
        return null;
    }
